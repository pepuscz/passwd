...
```bash
PASSWD_ORIGIN=https://your-deployment.passwd.team npx -y @passwd/passwd-cli@1.5.3 login
```
  ...
  secrets: {
    providers: {
      passwd: {
        ...
        args: ["-y", "@passwd/passwd-cli@1.5.3", "resolve"],
...
Store your API keys as secrets in passwd.team, then use their IDs in the `id` field. Run `npx @passwd/passwd-cli@1.5.3 list` to find them.
...
```bash
PASSWD_ORIGIN=https://your-deployment.passwd.team npx -y @passwd/passwd-agent-cli@1.5.3 login
```
...
CMD: `npx -y @passwd/passwd-agent-cli@1.5.3`
...
For multiple deployments, log in to each origin separately (`PASSWD_ORIGIN=... npx @passwd/passwd-agent-cli@1.5.3 login`). The agent can then switch with `--env` — see the Multi-environment section in the skill above.
...
```json
{
  "mcpServers": {
    "passwd": {
      "command": "npx",
      "args": ["-y", "@passwd/passwd-mcp@1.5.3"],
      "env": {
...
npx @passwd/passwd-agent-cli@1.5.3 login
npx @passwd/passwd-agent-cli@1.5.3 list
npx @passwd/passwd-agent-cli@1.5.3 exec --inject DB_PASS=SECRET_ID:password -- psql -h host -U app
```
...
```bash
export PASSWD_ORIGIN=https://your-deployment.passwd.team
npx @passwd/passwd-cli@1.5.3 login
npx @passwd/passwd-cli@1.5.3 list
npx @passwd/passwd-cli@1.5.3 --help
```

For multiple deployments, log in to each origin separately, then use `--env` to switch:

```bash
PASSWD_ORIGIN=https://acme.passwd.team npx @passwd/passwd-cli@1.5.3 login
PASSWD_ORIGIN=https://initech.passwd.team npx @passwd/passwd-cli@1.5.3 login
npx @passwd/passwd-cli@1.5.3 list --env acme
```
